...
@section('content')
    ...
    <div class="student-detailView-container">
        ...
        @foreach($user->languages ?? [] as $language)
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                <button class="w-full flex justify-between items-center p-4 bg-gray-100 hover:bg-gray-200 transition duration-300">
                    <p class="text-lg font-semibold text-gray-800">{{$loop->index + 1}}. {{$language->language}}</p>
                ...
                <div class="mx-auto bg-white p-8 border-gray-300 shadow-sm">
                    <div class="grid grid-cols-2 gap-y-3">
                        <div class="text-left font-medium">Dil:</div>
                        <div class="text-left font-bold">{{$language->language}}</div>

                        <div class="text-left font-medium">İmtahan adı:</div>
                        <div class="text-left font-bold">{{$language->exam}}</div>

                        <div class="text-left font-medium">Səviyyə:</div>
                        <div class="text-left font-bold">{{$language->level}}</div>

                        <div class="text-left font-medium">Puan:</div>
                        <div class="text-left font-bold">{{$language->point}}</div>

                        <div class="text-left font-medium">Dəyişdir:</div>
                        <div class="text-left font-bold">
                            <a href="{{ route('lang.edit', $language->id) }}" class="hover:bg-gray-100 rounded">
                        ...
                        <div class="text-left font-bold">
                            <form action="{{ route('lang.destroy', $language->id) }}" method="post">
                                {{ method_field('DELETE') }}
                                @csrf
        ...
        <a href="{{route('lang.create', $user->id)}}" class="addLangBoxBtn" type="button">
