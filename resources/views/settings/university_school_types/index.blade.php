...
@section('content')
    <div class="dashboard-body">
        ...
        <div class="table-universityList-container">
            <table>
                ...
                <tbody>
                @foreach($university_school_types as $key => $university_school_type)
                    <tr>
                        <td class="universityListNumber">{{$key + 1}}</td>
                        <td class="universityListName">{{$university_school_type->title}}</td>
                        <td class="universityListName">{{$university_school_type->university_education_level?->title}}</td>
                        <td class="universityListOthersButtons">
                            <button class="universityListEdit"
                                    onclick="openEditModal({{ json_encode($university_school_type) }})">
                                <img src="{{ asset('/') }}assets/images/pen.svg" alt="">
                            </button>

                            <form action="{{ route('university_school_types.destroy', $university_school_type->id) }}" method="POST"
    ...
    <x-pagination :paginator="$university_school_types"/>
    <div class="addUniversity-Modal" id="createModal" style="display: none;">
        <div class="addUniversity-Modal-box">
            ...
            <form action="{{ route('university_school_types.store') }}" class="form_addUniversity-Modal" method="POST">
                @csrf
                <div class="form-item">
    ...
    <div class="editUniversity-Modal" style="display: none;">
        <div class="editUniversity-Modal-box">
            ...
            <form id="editForm" method="POST" class="form_editUniversity-Modal">
                ...
                <div class="form-item">
                    <label for="">Təhsil pilləsini seçin</label>
                    <select id="editEducationLevel" name="university_education_level_id">
                        @foreach($university_education_levels as $university_education_level)
                            <option value="{{$university_education_level->id}}">{{$university_education_level->title}}</option>
                        @endforeach
                    </select>
    ...
    <script>
        ...
        function openEditModal(schoolType) {
            ...
            document.getElementById('editForm').action = `/university_school_types/${schoolType.id}`;

            let educationLevelSelect = document.getElementById("editEducationLevel");
            educationLevelSelect.value = schoolType.university_education_level_id;
