...
@section('title', 'Səlahiyətlər')

@section('content')

    <a href="{{route('permissions.index')}}" class="goBack">
    ...
    <div class="addNewPermission-container">
        <h2>Səlahiyət məlumatı</h2>
        <form action="{{route('permissions.update', $permission->id)}}" class="addNewPermissionForm" method="post">
            {{ method_field('PUT') }}
            @csrf
            <div class="form-items">
                <div class="form-item">
                    <label for="">Ad</label>
                    <input type="text" placeholder="Ad" name="name" value="{{$permission->name}}">
                </div>
                <div class="form-item">
                    <label for="">Qrup</label>
                    <input type="text" placeholder="Qrup adı" name="group_name" value="{{$permission->group_name}}">
                </div>
            </div>
            <button class="addPermissionBtn" type="submit">Düzəliş et</button>
        </form>
    </div>
