...
@section('content')
    ...
    <a href="{{route('permissions.create')}}" class="addNewPermission">
    ...
    <div class="table-permission-container">
        <table>
            ...
            @foreach($permissions as $key => $permission)
                <tr>
                    <td class="permissionNumber">{{$key + 1}}</td>
                    <td class="permissionName">{{$permission->name}}</td>
                    <td class="permissionGroup">{{$permission->group_name}}</td>
                    <td class="permissionOthersButtons">
                        <a href="{{route('permissions.edit', $permission->id)}}" class="permissionEdit">
                        ...
                        <form action="{{route('permissions.destroy', $permission->id)}}" method="post" style="display: inline-block">
                            {{ method_field('DELETE') }}
                            @csrf
                            <button class="permissionDelete" onclick="return confirm('Məlumatın silinməyin təsdiqləyin')" type="submit">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
