...
@section('content')
    ...
    <div class="addNewAgent-container">
        <h2>Agent məlumatlarını dəyiş</h2>
        <form action="{{route('agents.update', $agent->id)}}" class="addNewAgentForm" method="post" enctype="multipart/form-data">
            @csrf
            {{ method_field('PUT') }}
            <div class="form-items">
                <div class="form-item">
                    <label for="">Ad</label>
                    <input type="text" name="name" value="{{old('name', $agent->name)}}" placeholder="Ad">
                    @if($errors->first('name')) <small class="form-text text-danger">{{$errors->first('name')}}</small>@endif
                </div>
                <div class="form-item">
                    <label for="">Soyad</label>
                    <input type="text" name="surname" value="{{old('surname', $agent->surname)}}" placeholder="Soyad">
                    @if($errors->first('surname')) <small class="form-text text-danger">{{$errors->first('surname')}}</small>@endif
                </div>
                <div class="form-item">
                    <label for="">Mobil nömrə</label>
                    <input type="text" name="phone" value="{{old('phone', $agent->phone)}}" placeholder="+994 00 000 00 00">
                    @if($errors->first('phone')) <small class="form-text text-danger">{{$errors->first('phone')}}</small>@endif
                </div>
                <div class="form-item">
                    <label for="">Şirkət adı</label>
                    <input type="text" name="company_name" placeholder="Şirkət adı" value="{{old('company_name', $agent->agent_info?->company_name)}}">
                ...
                <div class="form-item">
                    ...
                    <input type="text" name="address" placeholder="Ünvan" value="{{old('address', $agent->agent_info?->address)}}">
                    @if($errors->first('address')) <small class="form-text text-danger">{{$errors->first('address')}}</small>@endif
                </div>
                <div class="form-item">
                    <label for="">Email</label>
                    <input type="email" name="email" value="{{old('email', $agent->email)}}" placeholder="Email">
