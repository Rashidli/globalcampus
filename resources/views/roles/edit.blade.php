@extends('layouts.master')
@section('title', 'Rollar')

@section('content')

    <a href="{{route('roles.index')}}" class="goBack">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M10.0303 6.46967C10.3232 6.76256 10.3232 7.23744 10.0303 7.53033L6.31066 11.25L14.5 11.25C15.4534 11.25 16.8667 11.5298 18.0632 12.3913C19.298 13.2804 20.25 14.7556 20.25 17C20.25 17.4142 19.9142 17.75 19.5 17.75C19.0858 17.75 18.75 17.4142 18.75 17C18.75 15.2444 18.0353 14.2196 17.1868 13.6087C16.3 12.9702 15.2133 12.75 14.5 12.75L6.31066 12.75L10.0303 16.4697C10.3232 16.7626 10.3232 17.2374 10.0303 17.5303C9.73744 17.8232 9.26256 17.8232 8.96967 17.5303L3.96967 12.5303C3.67678 12.2374 3.67678 11.7626 3.96967 11.4697L8.96967 6.46967C9.26256 6.17678 9.73744 6.17678 10.0303 6.46967Z" fill="black"/>
        </svg>
        Geri
    </a>
    <div class="addNewRolls-container">
        <h2>Roll məlumatı</h2>
        <form action="{{ route('roles.update', $role->id) }}" method="post" class="addNewRollsForm">
            @csrf
            {{ method_field('PUT') }}

            <!-- Role Name Input -->
            <div class="form-items">
                <div class="form-item">
                    <label for="role-name">Ad</label>
                    <input
                        type="text"
                        placeholder="Ad"
                        name="name"
                        id="role-name"
                        value="{{ $role->name }}"
                        class="form-control"
                    >
                    @if ($errors->first('name'))
                        <small class="form-text text-danger">{{ $errors->first('name') }}</small>
                    @endif
                </div>
            </div>

            <!-- Permissions Section -->
            <div class="rolls-checks-container">
{{--                <h3>Permissions</h3>--}}

                @foreach ($permissions as $groupName => $groupPermissions)
                    <div class="rolls-checks-box">
                        <!-- Group Name Checkbox -->
                        <input
                            id="{{ $groupName }}"
                            class="group-checkbox"
                            data-group="{{ $groupName }}"
                            type="checkbox"
                        >
                        <label style="font-weight: 600" for="{{ $groupName }}">{{ $groupName }}</label>

                        <div class="rolls-checks-list">
                            @foreach ($groupPermissions as $permission)
                                <!-- Individual Permissions -->
                                <div class="rolls-check-item">
                                    <input
                                        id="permission-{{ $permission->id }}"
                                        type="checkbox"
                                        data-group="{{ $groupName }}"
                                        name="permission[]"
                                        value="{{ $permission->name }}"
                                    @foreach ($role->permissions as $p)
                                        {{ $p->name == $permission->name ? 'checked' : '' }}
                                        @endforeach
                                    >
                                    <label for="permission-{{ $permission->id }}">{{ $permission->name }}</label>
                                </div>
                            @endforeach
                        </div>
                    </div>
                @endforeach
            </div>

            <!-- Submit Button -->
            <div class="form-actions">
                <button class="addRollsBtn" type="submit">Düzəliş et</button>
            </div>
        </form>
    </div>

@endsection
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
<script>
    $(document).ready(function () {
        $('.group-checkbox').on('click', function () {
            // Get the state of the clicked group checkbox
            var groupBoolean = $(this).prop('checked');
            var groupValue = $(this).data('group');

            // Toggle all checkboxes within the same group
            $('input[type="checkbox"][data-group="' + groupValue + '"]').each(function () {
                $(this).prop('checked', groupBoolean);
            });
        });

        // Optional: Add an event listener for individual checkboxes to update the group checkbox state
        $('input[type="checkbox"][name="permission[]"]').on('click', function () {
            var groupValue = $(this).data('group');

            // Check if all checkboxes in the group are checked
            var allChecked = $('input[type="checkbox"][data-group="' + groupValue + '"]:not(.group-checkbox)').length ===
                $('input[type="checkbox"][data-group="' + groupValue + '"]:not(.group-checkbox):checked').length;

            // Update the group checkbox based on individual checkboxes
            $('.group-checkbox[data-group="' + groupValue + '"]').prop('checked', allChecked);
        });
    });

</script>
